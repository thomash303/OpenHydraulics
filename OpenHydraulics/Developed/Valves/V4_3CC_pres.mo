...
model V4_3CC_pres
  ...
  // Enabling parameters
  parameter Boolean filterEnable = true "Enable min/max filtering of the input signal (0-1). Strongly recommend to enable." annotation(
  ...
  parameter Boolean responseEnable = false "Enable dynamic (second-order) response" annotation(
    Dialog(tab = "Dynamic Response", enable = manualValveControl),
    choices(checkBox = true));
  // Dynamic response parameters
  parameter SI.Frequency bandwidth = 10 "Bandwidth of 2nd order response" annotation(
    Dialog(tab = "Dynamic Response", enable = responseEnable));
  parameter Real dampingCoeff = 1 "Damping coefficient of 2nd order response" annotation(
    Dialog(tab = "Dynamic Response", enable = responseEnable));
  // Flow coefficient
  ...
  replaceable function valveCharacteristic = BaseClasses.ValveCharacteristics.linear constrainedby BaseClasses.ValveCharacteristics.baseFun "Valve flow characteristic" annotation(
     Dialog(group = "Valve Characteristics"),
