...
model w2w_wec
  ...
  parameter Modelica.Units.SI.Voltage VNominal = 200 "Nominal RMS voltage per phase";
  ...
  OceanEngineeringToolbox.Hydro.HydrodynamicBody sphere(I_11 = 20907301, I_22 = 21306091, I_33 = 37085481, animationEnable = true, bodyColour = {0, 255, 0}, bodyIndex = 1, enableDampingDragForce = false, enableExcitationForce = true, enableHydrostaticForce = true, enableRadiationForce = true, geometryFile = "file://C:/Users/thogan1/Documents/GitHub/OceanEngineeringToolbox/applications/Validation/NL_hydro/nlHydro/Meshes/sphere_9024p.stl", ra_CM = {0, 0, 0.7}, Ad = {0, 0, 5, 0, 0, 0}, Cv = {0, 0, 1e5, 0, 0, 0}, enableMorisonForce = true) annotation(
  ...
  inner OceanEngineeringToolbox.DataImport.FileDirectory fileDirectory(file = "C:/Users/thogan1/Documents/GitHub/OceanEngineeringToolbox/applications/Validation/W2W/hydro/Radiation_heave_only/SPHERE_3480phydroCoeff_FOAMM.mat") annotation(
    Placement(transformation(origin = {10, 76}, extent = {{-9, -7.25}, {9, 7.25}})));
  inner OceanEngineeringToolbox.Environmental.Environment environment(redeclare IrregularWave wave(file = fileDirectory.file, localSeedPhase = 31506, globalSeedPhase = 64503) "Irregular wave", Trmp = 10) annotation(
    Placement(transformation(origin = {-24, 76}, extent = {{-10, -8}, {10, 8}})));
  ...
  Modelica.Units.SI.Force Fpto = doubleActingCylinder.boreDiameter^2/4*3.14*(hpAccumulator.port_a.p - lpAccumulator.port_a.p)*sign(sphere.body.absoluteSensor.v[3]);
  ...
  Modelica.Electrical.Machines.BasicMachines.InductionMachines.IM_SquirrelCage SCIG(Jr = SCIGData.Jr, Js = SCIGData.Js, Lm = SCIGData.Lm, Lrsigma = SCIGData.Lrsigma, Lssigma = SCIGData.Lssigma, Lszero = SCIGData.Lszero, Rr = SCIGData.Rr, Rs = SCIGData.Rs, TrOperational = 293.15, TrRef(displayUnit = "K") = 0, TsOperational = 293.15, TsRef(displayUnit = "K") = 0, alpha20r = 0, alpha20s(displayUnit = "1/K") = 0, frictionParameters(PRef = 0.02, wRef (displayUnit = "rad/s")= 1, power_w = 1), fsNominal = SCIGData.fsNominal, p = SCIGData.p, phiMechanical(fixed = false, start = 0), statorCoreParameters(VRef = 0), strayLoadParameters(IRef = 0, power_w = 0), wMechanical(fixed = false, start = 0)) annotation(
  ...
  parameter Modelica.Electrical.Machines.Utilities.ParameterRecords.IM_SquirrelCageData SCIGData "Induction machine data" annotation(
    Placement(transformation(origin = {84, 148}, extent = {{-20, -80}, {0, -60}})));
  ...
  Modelica.Blocks.Sources.Ramp referenceSpeed(duration = 0.15, height = 157.5) annotation(
    Placement(transformation(origin = {122, -56}, extent = {{8, -8}, {-8, 8}})));
 //Modelica.Blocks.Math.Feedback feedback1 annotation(
  ...
  Modelica.Blocks.Continuous.LimPID PID(controllerType = Modelica.Blocks.Types.SimpleController.PI, Ti = 2, Td = 0.1, yMax = 1, yMin = 0.1, k = 0.1, Ni = 0.6)  annotation(
...
equation
  ...
  connect(terminalBox.plug_sn, SCIG.plug_sn) annotation(
    Line(points = {{124, -12}, {124, -12}}, color = {0, 0, 255}));
  connect(terminalBox.plug_sp, SCIG.plug_sp) annotation(
...
//daq.Fexc = sphere.excitation.excitationForce.F[3];
//daq.Fpto = Fpto;
...
// Energy
  ...
  daq.Pelec = SCIG.powerBalance.powerStator;
  daq.Pgen_fric = SCIG.powerBalance.lossPowerFriction;
  daq.Pgen_cop = SCIG.powerBalance.lossPowerStatorWinding + SCIG.powerBalance.lossPowerRotorWinding;
  daq.Pgen_mech = SCIG.powerBalance.powerMechanical;
  connect(rotSpeedSensor.flange, variableDisplacementMotor.flange_a) annotation(
    Line(points = {{70, -84}, {70, -22}}));
  ...
  connect(torqueSensor.flange_b, SCIG.flange) annotation(
  ...
  connect(pHP.port_a, hpAccumulator.port_a) annotation(
    Line(points = {{3, 34}, {3, 18}, {28, 18}}, color = {255, 0, 0}));
  ...
  connect(mbAbsoluteSensor.frame_a, sphere.frame_b) annotation(
    Line(points = {{-32, 36}, {-32, 35.5}, {-38, 35.5}, {-38, 37}}, color = {95, 95, 95}));
  connect(motorFlow.port_b, variableDisplacementMotor.portP) annotation(
    Line(points = {{62, 18}, {62, -14}}, color = {255, 0, 0}));
  connect(referenceSpeed.y, PID.u_s) annotation(
    Line(points = {{113, -56}, {100, -56}}, color = {0, 0, 127}));
  connect(pB.port_a, doubleActingCylinder.port_b) annotation(
    Line(points = {{-32, -6}, {-32, -14}}, color = {255, 0, 0}));
  connect(doubleActingCylinder.port_b, v4_3cc.portB) annotation(
    Line(points = {{-32, -14}, {-18, -14}, {-18, -18}, {-16, -18}}, color = {255, 0, 0}));
  connect(doubleActingCylinder.port_a, v4_3cc.portA) annotation(
    Line(points = {{-32, -30}, {-18, -30}, {-18, -26}, {-16, -26}}, color = {255, 0, 0}));
  connect(pA.port_a, doubleActingCylinder.port_a) annotation(
    Line(points = {{-32, -36}, {-32, -30}}, color = {255, 0, 0}));
  connect(pistonInflow.port_a, lpAccumulator.port_a) annotation(
    Line(points = {{0, -66}, {26, -66}}, color = {255, 0, 0}));
  connect(pLP.port_a, lpAccumulator.port_a) annotation(
    Line(points = {{47, -66}, {26, -66}}, color = {255, 0, 0}));
  connect(variableDisplacementMotor.portT, lpAccumulator.port_a) annotation(
    Line(points = {{62, -30}, {62, -66}, {26, -66}}, color = {255, 0, 0}));
  connect(PID.y, variableDisplacementMotor.dispFraction) annotation(
    Line(points = {{77, -56}, {77, -28}, {70, -28}}, color = {0, 0, 127}));
  connect(rotSpeedSensor.w, PID.u_m) annotation(
    Line(points = {{91, -84}, {88, -84}, {88, -68}}, color = {0, 0, 127}));
  annotation(
    ...
    Diagram(coordinateSystem(extent = {{-120, 100}, {240, -120}})));
end w2w_wec;
