...
model CC_HIL_sens_no_rot_val_mot_boost_coef
  ...
  parameter Modelica.Units.SI.Area Av = 4.86-6;
  ...
  OpenHydraulics.Custom.Basic.LaminarRestriction laminarRestriction(D = 1, L = 0.01, p_init(displayUnit = "bar") = 2e5) annotation(
...
equation
  ...
  connect(lpAccumulator.port_a, laminarRestriction.port_a) annotation(
    Line(points = {{32, -12}, {50, -12}}, color = {255, 0, 0}));
  connect(laminarRestriction.port_b, constantDisplacementPump.portP) annotation(
    Line(points = {{70, -12}, {66, -12}, {66, 4}}, color = {255, 0, 0}));
  annotation(
    ...
    Diagram);
end CC_HIL_sens_no_rot_val_mot_boost_coef;
