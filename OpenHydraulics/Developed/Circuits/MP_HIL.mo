...
model MP_HIL
  ...
  Developed.Machines.ConstantDisplacementPump constantDisplacementPump(CMotorLeakage = 0.00025, Dconst = 4e-6*17, p_init(displayUnit = "bar") = 3.5e6) annotation(
    Placement(transformation(origin = {58, 14}, extent = {{10, 10}, {-10, -10}})));
  Developed.Volumes.Accumulator hpAccumulator(gasVolume = 3.8e-2, initType = Developed.Types.AccInit.Volume, liquidVolume = 2.8e-2, p_init(displayUnit = "bar") = 3.5e6, p_precharge(displayUnit = "bar") = 1e6, p_max = 2e8, V_init = 2.285e-2) annotation(
  ...
  Developed.Valves.V4_3CC v4_3cc(p_crack(displayUnit = "bar") = 35000, p_init(displayUnit = "bar") = 1e6, p_open(displayUnit = "bar") = 4e4, CvData = Modelica.Fluid.Types.CvTypes.Av, Av = 1e-4, Cd = 1) annotation(
  ...
  OpenHydraulics.Developed.Machines.ConstantDisplacementPump constantDisplacementPump1(CMotorLeakage = 0.00025, Dconst = -4e-6*0, p_init(displayUnit = "bar")) annotation(
    Placement(transformation(origin = {-50, -68}, extent = {{10, -10}, {-10, 10}})));
  Valves.CheckValve checkValve(manualValveControl = false, p_crack = 1000, p_open = 2000, CvData = Modelica.Fluid.Types.CvTypes.Av, Av = 0.001, p_init = 2e5) annotation(
    Placement(transformation(origin = {-72, -46}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  Volumes.OpenTank tank annotation(
  ...
  Sources.ConstPressureSource constPSource annotation(
