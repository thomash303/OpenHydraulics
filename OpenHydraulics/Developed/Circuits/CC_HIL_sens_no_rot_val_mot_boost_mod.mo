...
model CC_HIL_sens_no_rot_val_mot_boost_mod
  ...
  Developed.Volumes.Accumulator hpAccumulator(gasVolume = 3.8e-3, initType = Developed.Types.AccInit.Volume, liquidVolume = 2.8e-3, p_init(displayUnit = "bar") = 3.5e6, p_precharge(displayUnit = "bar") = 1e6, p_max = 2e8, V_init = 2.045e-3+ 0.05e-3) annotation(
    Placement(transformation(origin = {44, 44}, extent = {{-10, -10}, {10, 10}})));
  Developed.Volumes.Accumulator lpAccumulator(gasVolume = 1e-3, initType = Developed.Types.AccInit.Volume, liquidVolume = 0.8e-3, p_init(displayUnit = "bar") = 2e5, p_precharge(displayUnit = "bar") = 2e5, p_max = 5e7, V_init = 0.6e-3+ 0.15e-3) annotation(
  ...
  parameter Modelica.Units.SI.Area Av = 2.9698e-5 - 2.1e-5 - 0.3e-6;
  parameter Real Cd = 1;
  ...
  Developed.Valves.ReliefValve reliefValve1(Av = 0.001, Cd = 1, CvData = Modelica.Fluid.Types.CvTypes.Av, p_open = 7.5e5, p_relief = 7e5) annotation(
...
equation
  ...
  annotation(
    ...
    Diagram);
end CC_HIL_sens_no_rot_val_mot_boost_mod;
