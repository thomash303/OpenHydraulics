...
model cylinder_driving
  ...
  Cylinders.DoubleActingCylinder doubleActingCylinder(strokeLength = 0.4, q_nom = 1e-4, pistonRodMass = 0.0002, s_init = 0.185, leakageEnable = false, stribeckFrictionEnable = false, Cv = 0, f_st = 1, f_c = 0, compressibleEnable = false, CRodExLeakage = 0.001, initType = OpenHydraulics.Developed.Types.RevoluteInit.Position, gravityAccelerationEnable = false)  annotation(
  ...
  OpenHydraulics.Components.Volumes.CircuitTank circuitTank annotation(
    Placement(transformation(origin = {46, 28}, extent = {{30, -90}, {10, -70}})));
equation
  ...
  connect(fixed.flange, doubleActingCylinder.flange_a) annotation(
    Line(points = {{-12, 50}, {44, 50}}, color = {0, 127, 0}));
  connect(j1.port[2], doubleActingCylinder.port_a) annotation(
    Line(points = {{28, 0}, {56, 0}, {56, 42}, {46, 42}}, color = {255, 0, 0}));
  connect(sine.y, position.phi_ref) annotation(
    Line(points = {{-67, 8}, {-48, 8}, {-48, -36}}, color = {0, 0, 127}));
  connect(fluidPower2MechRotConst.port_a, circuitTank.port_b) annotation(
    Line(points = {{6, -44}, {56, -44}, {56, -52}}, color = {255, 0, 0}));
  connect(circuitTank.port_a, doubleActingCylinder.port_b) annotation(
    Line(points = {{76, -52}, {62, -52}, {62, 42}}, color = {255, 0, 0}));
  annotation(
