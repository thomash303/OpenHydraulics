...
model StribeckFriction_stateHandle "Stribeck friction"

/* Modelica approach to handle friction "cleanly, but causes a lot of numerical issues */
...
// Relative acceleration
  ...
  a_relfric = a_rel;
  f0 = (f_c + f_st);
// Variables defined in partial friction, defined similar to MassWithStopAndFriction
  f0_max = f0*1.001;
// Variables defined in partial friction, defined similar to MassWithStopAndFriction
  free = f0 <= 0 and Modelica.Units.Cv <= 0;
// Variables defined in partial friction, defined similar to MassWithStopAndFriction
// Total friction force (handled "cleanly" by state events)
  f = if locked then sa*unitForce else if free then 0 else (if startForward then Modelica.Units.Cv*v_rel + f_c + f_st else if startBackward then Modelica.Units.Cv*v_rel - f_c - f_st else if pre(mode) == Forward then Modelica.Units.Cv*v_rel + f_c + f_st*exp(-abs(v_rel)/Cst) else Modelica.Units.Cv*v_rel - f_c - f_st*exp(-abs(v_rel)/Cst));
  annotation(
...
end StribeckFriction_stateHandle;
