...
model FluidPower2MechTrans
  ...
  parameter Modelica.Units.SI.AbsolutePressure maxPressure = 3e7 "Maximum rated pressure" annotation(
    Dialog(tab = "Sizing"));
  parameter Modelica.Units.SI.Volume residualVolume = 1e-6 "Volume remaining when s_rel<=0" annotation(
    Dialog(tab = "Sizing"));
  parameter Real stopStiffness(final unit = "N/m", final min = 0) = maxPressure*A*A/residualVolume "stiffness when chamber becomes empty" annotation(
    Dialog(tab = "Dynamics"));
  parameter Real stopDamping(final unit = "N.s/m", final min = -1000) = stopStiffness/10 "damping when chamber becomes empty" annotation(
    Dialog(tab = "Dynamics"));
